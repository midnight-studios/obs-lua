...
function script_update(settings)
	...
	if media_name ~= p_media_name and p_media_name ~= nil then
		--[[
				media source changed, remove this callback
		]]	
		disconnect_source_signal()
	end
	--[[
			
	]]		
	if media_name ~= "Select" and scene_name ~= "Select" then
		--[[
...
function disconnect_source_signal()
	--[[
		Sources have unique names
		Load the Source, from the Name
	]]
	obs.signal_handler_disconnect( sh_source, "media_ended", media_ended )	
end
--[[
--------------------------------------------------------------------
 custom function
...
function reconnect_source_signal()
	...
	local source = obs.obs_get_source_by_name( media_name ) -- Increments the source reference counter, use obs_source_release() to release it when complete. 
	--[[
		Found Source:
	]]
	if source ~= nil then  
		...
		sh_source = obs.obs_source_get_signal_handler( source )	
			--[[
			...
			obs.signal_handler_connect( sh_source, "media_ended", media_ended )	
	end  
	--[[
...
function connect_source_signal( cd )
	--[[	
		Fetch source name from properties
	]]		
	media_name = obs.obs_data_get_string(p_settings, "media_name")
...
function media_ended( cd )
	...
	--[[
		Fetch scene name from properties
	]]	
	scene_name = obs.obs_data_get_string( p_settings, "scene_name" )
	--[[
		load source from scene name
    ...
    if source ~= nil then
	--[[
		Set OBS Active Scene to the selected scene
	]]	
