...
function script_tick(sec)
	...
	if cur_ns < 1 and (mode == "Countdown" or mode == "Countdown then Countup" or mode == "Specific time" or mode == "Specific date and time") then
				
		if mode ~= "Countdown then Countup"  and next_scene == "select" then
			set_time_text(cur_ns, stop_text)
			stop_timer()
			return
			
		elseif next_scene ~= "select" then
...
function settings_modified(props, prop, settings)
	...
	if (mode_setting == "Countdown" or mode == "Countdown then Countup") then
...
function script_properties()
	...
	obs.obs_property_list_add_string(p_mode, "Countdown then Countup", "downandup")
	...
	local scene_list = obs.obs_properties_add_list(props, "next_scene", "Scene:", obs.OBS_COMBO_TYPE_LIST, obs.OBS_COMBO_FORMAT_STRING)
