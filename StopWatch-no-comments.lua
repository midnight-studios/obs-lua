...
function delayed_recording()
	obs.timer_add( recording_callback, 100 ) 
end	
	...
	if source ~= nil then   
		...
		local state = obs.obs_source_media_get_state( source )
			...
			if get_source_looping( source_name ) then
				...
				if ref == "caution" then
					obs.timer_add( timer_caution_media_end_callback, math.floor( media[ref..'_duration'] * 1000 ) ) 
				end
			
				if ref == "warning" then
					obs.timer_add( timer_warning_media_end_callback, math.floor( media[ref..'_duration'] * 1000 ) ) 
				end
