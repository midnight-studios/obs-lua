...
function script_description()
	return "When a Group Source (Scene Item) visibility is toggled all children source items will also be toggled. The user can specify Scene Items (Sources) to be exlused from the toggle action."
end
		...
		if isn == sourceName then
			...
			if not in_exclusion_array(sourceName) and not in_exclusion_array(isn) then 
				...
				for j, groupitem in ipairs(groupitems) do
					...
					local grp_item_src = obs.obs_sceneitem_get_source(groupitem)

					local gisn = obs.obs_source_get_name(grp_item_src)
					
						if not visible then

							obs.obs_sceneitem_set_visible(groupitem, false)
							--obs.obs_source_set_enabled(obs.obs_get_source_by_name(gisn), false)

						else
							--obs.obs_source_set_enabled(obs.obs_get_source_by_name(gisn), true)
