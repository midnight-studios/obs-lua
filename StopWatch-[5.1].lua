...
Published / Released: 2023-08-01 20:26
...
function assign_marker_note_source_name( ref )
	...
	if note_source_unversioned_id ~= "group" then
		...
		for i, source_name in pairs( data_list ) do
			if is_valid_type_source( source_name, "text_gdiplus" ) then
		...
		--log( pre_dump( media["used_note_source_".. ref] ))
		--log( pre_dump( media["used_note_source_".. ref] ))
...
function get_minutes_allocation( str )
    ...
    local index, _ = string.find( instructions, "M%d+" )
    ...
    return tonumber( string.sub( instructions, index + 1 ) )
end
--[[
----------------------------------------------------------------------------------------------------------------------------------------
...
local function get_text_color( source_name )
	...
	if source ~= nil then -- continue if we have a source
		...
		if source_id == "text_gdiplus" or source_id == "text_ft2_source" then -- identify text type sources only
			...
			color = obs.obs_data_get_int( settings, "color" ); -- update source settings
		end
	end
...
local function hide_note_a_timer_callback()
	...
	if media["note_source_unversioned_id_marker_a"] == "text_gdiplus" then
...
local function hide_note_b_timer_callback()
	...
	if media["note_source_unversioned_id_marker_b"] == "text_gdiplus" then
...
local function set_text_note_color( ref )
	...
	if media["note_source_unversioned_id_" .. ref] ~= "text_gdiplus" then return end
...
local function time_mark_check( ref )
	...
	if raw_time( round_seconds, true ) == text_marker and not media["activated_".. ref] then -- compare current time with marker
		...
		if enable_marker_notes ~= 1 and ref == "marker_a" then -- marker notes is enabled and the input reference matches	
			...
			if media["note_source_unversioned_id_" .. ref] == "text_gdiplus" then
...
local function reset_cycle_source_list()
	...
	if in_table( {"Auto List"}, timer_expire_event ) and not in_table( ignore_list, cycle_source_children ) then
		...
		if source_name == nil or in_table( ignore_list, source_name ) == nil or source_list == nil then 	
...
local function pause( pressed, force_activate )
    debug_log( 'pause(' .. pre_dump(pressed) .. ') -- function variable names:  pressed ' )
	if not pressed then 
		return;
	end
...
local function property_onchange( props, property, settings )
	...
	obs.obs_property_set_visible( sub_limit_note_source_visible_prop, sub_limit_note_source ~= "select" and not in_table( ignore_list, timer_source_value ) );
	--[[
...
local function timer_expire_event_cycle_source_list( source_type )
	...
	if timer_mode == 2 and timer_expire_event == "Source List" then
		local source_list = get_cycle_source_list( source_type );
		if source_list == nil then 	
			cycle_index, source_list = cycle_source_list_by_source_type( source_list, "source", cycle_index );
		end	
	end	
	--[[
		The folowing section will only update cycle_index
	...
	if timer_mode == 2 and timer_expire_event == "Scene List" then
		local source_list = get_cycle_source_list( source_type );
		if source_list == nil then 	
			cycle_index, source_list = cycle_source_list_by_source_type( source_list, "scene", cycle_index );
		end	
	end	
	--[[
		The folowing section will only update cycle_index
	...
	if timer_expire_event == "Auto List" and not in_table( ignore_list, cycle_source_children ) then
		local source_name = cycle_source_children
		local source_list = list_source_children_by_source_name( source_name )
		if source_name == nil or in_table( ignore_list, source_name ) == nil or source_list == nil then 	
			cycle_index, source_list = cycle_source_list_by_source_name( source_list, cycle_index );
	...
	--timer_active = false
	pause( true, true ); -- Restart the timer
end
--[[
----------------------------------------------------------------------------------------------------------------------------------------
	Description:	
...
function timer_ended( source_name )
	...
	if timer_mode == 2 and in_table( { "Scene List", "Source List", "Auto List" }, timer_expire_event ) then
		timer_expire_event_cycle_source_list();
	end	
...
function script_properties()
    ...
    local p_64_9 = obs.obs_properties_add_bool( group_props_4, "enable_property_timer_manipulation", "Timer Manipulation" );
    ...
    local p_64_10 = obs.obs_properties_add_bool( group_props_4, "enable_property_debug", "Debugging" );
    ...
    local p_64_11 = obs.obs_properties_add_bool( group_props_4, "enable_property_backup", "Backup" );
