...
function script_properties()
	...
	list = {}
	local property_sn = obs.obs_properties_add_list( ctx.propsDef, "search_source_name", "Source Name", obs.OBS_COMBO_TYPE_EDITABLE, obs.OBS_COMBO_FORMAT_STRING )
...
function script_defaults( settings )
	...
	obs.obs_data_set_default_string( settings, "search_source_name", list_all )
...
function script_update( settings )
	...
	search_source_name = obs.obs_data_get_string( settings, "search_source_name" )
...
function property_onchange( props, property, settings )
	...
	if prop_name == "source_type" then
		...
		obs.obs_data_set_string( settings, "search_source_name", list_all ) -- Don't allow timer and active text source to be the same
	end	
	...
	for key, value in pairsByKeys( list ) do
		obs.obs_property_list_add_string( search_source_name_props, value, value )
	end
...
function getSourcesTable()
    ...
    local function process_scene_items(scene_items, parent_name)
        ...
        for _, scene_item_value in ipairs(scene_items) do
            ...
            local scene_item_source_name = obs.obs_source_get_name(source)
            local source_enabled = obs.obs_source_enabled(source)
            local group = obs.obs_group_from_source(source)
            local scene_item_id = obs.obs_source_get_id(source)
            local scene_item_unversioned_id = obs.obs_source_get_unversioned_id(source)
            local scene_item_display_name = obs.obs_source_get_display_name(scene_item_id)
            local scene_item_type = obs.obs_source_get_type(source)
            local filter_count = obs.obs_source_filter_count(source)
            local filters = obs.obs_source_enum_filters(source)
            local scene_item_visible = obs.obs_sceneitem_visible(scene_item_value)
            local scene_item_uuid = obs.obs_source_get_uuid(source)
            local source_info = {
                position = group and "group" or "source",
                nested_scene = (scene_item_type == obs.OBS_SOURCE_TYPE_SCENE and not group),
                type = scene_item_type,
                id = scene_item_id,
                uuid = scene_item_uuid,
                visible = scene_item_visible,
                unversioned_id = scene_item_unversioned_id,
                display_name = scene_item_display_name,
                source_name = scene_item_source_name,
            ...
            if group then
                ...
                if group_items then
                    source_info.sources = process_scene_items(group_items, scene_item_source_name)
    ...
    local function process_scenes(scenes, parent_name)
        local scene_list = {}
        for _, scene_source in pairs(scenes) do
            local scene = obs.obs_scene_from_source(scene_source)
            local scene_items = obs.obs_scene_enum_items(scene)
            local scene_source_name = obs.obs_source_get_name(scene_source)
            local scene_enabled = obs.obs_source_enabled(scene_source)
            local scene_id = obs.obs_source_get_id(scene_source)
            local scene_unversioned_id = obs.obs_source_get_unversioned_id(scene_source)
            local scene_display_name = obs.obs_source_get_display_name(scene_id)
            local scene_type = obs.obs_source_get_type(scene_source)
            local filter_count = obs.obs_source_filter_count(scene_source)
            local filters = obs.obs_source_enum_filters(scene_source)
            local scene_uuid = obs.obs_source_get_uuid(scene_source)
            local scene_uuid = obs.obs_source_get_uuid(scene_source)
            local scene_info = {
                position = "scene",
                nested_scene = false,
                type = scene_type,
                id = scene_id,
                unversioned_id = scene_unversioned_id,
                display_name = scene_display_name,
                source_name = scene_source_name,
                source_enabled = scene_enabled,
                uuid = scene_uuid,
                ...
                filters = iterate_filters(filters),
                sources = process_scene_items(scene_items, scene_source_name)  -- Nest scene items data
            }
            table.insert(scene_list, scene_info)
    ...
    if current_collection then
        ...
        local scenes = obs.obs_frontend_get_scenes()
        if scenes then
            collection_info.children = #scenes
            collection_info.sources = process_scenes(scenes, current_collection)
            obs.source_list_release(scenes)
        end
        table.insert(source_list, collection_info)
    end
    return source_list
end
--[[
...
function doSearch( do_search )
    ...
    if scenes ~= nil then
        ...
        for key_scenesource, value_scenesource in pairs( scenes ) do
            ...
            for key_sceneitem, value_sceneitem in pairs( sceneitems ) do
				...
				--[[
						search_source_name is the value we are searching for
				]]		
				if search_source_name == source_name_parent then
					--[[
				...
				end	-- search_source_name == source_name_parent
				--[[
				]]
				if search_source_name == list_all then		
					if search_param == 1 then
				...
				end -- search_source_name == list_all
				if group ~= nil then
					local groupitems = obs.obs_scene_enum_items( group )	
					if groupitems ~= nil then
						for key_groupitem, value_groupitem in pairs( groupitems ) do
							...
							if search_source_name == source_name_group then
								if search_param == 1 then
										found_scenename = scenename
	...
	prefix_results = prefix_results .. string.format( "Search Type:%s \n", source_type );
	prefix_results = prefix_results .. string.format( "Search Name:%s \n", search_source_name );
	prefix_results = prefix_results .. string.format( "Search Params:%s \n", search_param );
    prefix_results = search_param ~= 1 and prefix_results .. string.format( "Filter Type:%s \n", filter_type ) or prefix_results ;
	prefix_results = search_param ~= 2 and prefix_results .. string.format( "Sources Found:%s \n", sources_found ) or prefix_results .. string.format( "Sources: %s \n", "Not requested or included in search results." ) ;
	prefix_results = search_param ~= 1 and prefix_results .. string.format( "Filters Found:%s \n", filters_found ) or prefix_results .. string.format( "Filters: %s \n", "Not requested or included in search results." ) ;
	...
	if output_results then 
        local r =  format_message( results )
        write_to_file( "txt", r, output_folder ); 
    end
